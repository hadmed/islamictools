...
//	};
	...
	static double getRiseSet (final Location loc, final Astro tastro, int type)
	{
	    ...
	    lhour =  limitAngle180 (( RAD_TO_DEG (mMath.acos (part3))));
	    ...
	    sunAlt = RAD_TO_DEG(mMath.asin (  Math.sin(rLat) * Math.sin(rB) 
	...
	static void computeAstroDay(double JD, AstroDay astroday)
	{
	    ...
	    RA = limitAngle(RAD_TO_DEG(mMath.atan2(RAn,RAd)));

	    DEC = mMath.asin( Math.sin(rGg) * Math.cos(rE) + Math.cos(rGg) * Math.sin(rE) * 
	    		Math.sin(rLambda));
	...
	static void computeTopAstro(final Location loc, final Astro astro, Astro topAstro)
	{
	    ...
	    for (i=0; i<3; i++)
	    {
	        ...
	        tU =mMath.atan (0.99664719 * Math.tan(rLat));
	        ...
	        tDEC = RAD_TO_DEG(mMath.atan2((Math.sin(astro.dec[i]) - tSin * Math.sin(SP)) * Math.cos(tRA0), 
